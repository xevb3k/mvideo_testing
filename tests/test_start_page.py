...
def test_homepage_is_opened(web_driver, window_width, window_height):
    ...
    assert page.driver.current_url == MVideoLinks.home_page_url, error_main_page_not_loaded
...
def test_logo_click(web_driver_desktop):
    ...
    page.logo_img.click()
    assert page.driver.current_url == MVideoLinks.home_page_url, "при клике на логотип не происходит переход на главную страницу"
...
def test_link_blog_m_click(web_driver_desktop):
    ...
    assert page.get_relative_link() == MVideoLinks.link_home_page_blog_m_click, 'ошибка при клике на ссылку "Блог М.Клик"'
...
def test_link_m_club(web_driver_desktop):
    ...
    assert page.get_relative_link() == MVideoLinks.link_home_page_m_club, 'ошибка при клике на ссылку "M.Club"'
...
def test_link_for_business(web_driver_desktop):
    ...
    assert page.get_relative_link() == MVideoLinks.link_home_page_for_business, 'ошибка при клике на ссылку "Для бизнеса"'
  

@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['смартфоны'],
                         ids=['ru'])
def test_search_positive(web_driver_desktop, search_text):
    """Тест проверяет работу поиска с разными данными """
    page = HomePage(web_driver_desktop)
    page.search_field_put_value(search_text)
...
@pytest.mark.parametrize("search_text", [StringGenMethods.generate_string(255), StringGenMethods.generate_string(1001),
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['пылесос', 'чайник со свистком', 'телевизор 4K'],
                                    ids=['ru', 'more_ru', 'ru_en'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['пылесос'],
                         ids=['ru'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['чайник со свистком'],
                         ids=['ru'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['чайник', 'телевизор'],
                         ids=['ru', 'ru2'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['чайник', 'пылесос', 'телевизор', 'холодильник'])
@pytest.mark.parametrize("min_value", ['100', '999', '1000', '49999'])
@pytest.mark.parametrize("max_value", ['500', '4999', '99999'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['чайник'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['ноутбук', 'картридж'])
...
@pytest.mark.smoke
@pytest.mark.positive
@pytest.mark.parametrize("search_text", ['холодильник'])
def test_search_products_filter_min_max(web_driver_desktop, search_text):
    ...
    min_price = input_min_value.get_attribute('value').replace(' ', '')
    max_price = input_max_value.get_attribute('value').replace(' ', '')
    assert prompt_max_price == min_price and prompt_max_price == max_price, 'система дает внести в поле фильтра макс цену больше максимальной предполагаемой'
...
def test_catalog(web_driver_desktop):
    ...
    if not catalog1:
        assert False, 'Каталог не раскрыт'
    else:
